def create_rep_code_stim_string(distance, rounds, p):
    ...
    n = 2*distance - 1 # total number of qubits
    qs = ' '.join(map(str, range(n))) # all qubits indices
    ms = ' '.join(map(str, range(1, n, 2))) # measure qubits indices
    ds = ' '.join(map(str, range(0, n, 2))) # data qubits indices
    down = ' '.join(map(str, range(n-1)))
    up = ' '.join(map(str, reversed(range(1, n))))
    nm = distance-1 # number of measure qubits
    nd = distance # number of data qubits
    ...
    for i, j in enumerate(range(1, n, 2)):
        string += f'DETECTOR({j-1}, 0) rec[{-nm - nd + i}] rec[{-nd + i}] rec[{-nd + i + 1}]\n'
